...
if($_GET['type'] == 'successform'){
    $a  = $_POST['TransactionID'];
    $b  = $_POST['alasan'];
    
    $updatekurirjobs = $config->runQuery("UPDATE kurir_jobs SET StatusKirim = 2, Notes ='". $b ."' WHERE TransactionNumber ='". $a ."' ");
    $updatekurirjobs->execute();
    ...
    if($updatekurirjobs) {
        ...
        $updatetransaction = $config->runQuery("UPDATE transaction SET statusOrder = '3', notes = '". $b ."'   WHERE transactionID ='". $a ."' ");
        $updatetransaction->execute();
        ...
        if($updatetransaction) {
            ...
            if($data['statusOrder'] == 3) {
                $SendStatus = '<tr>
                <td width="600" align="center" class="w640">
                <span class="article-content" style="font-family:Arial; font-size:24px;color:#333333; font-weight:bold; line-height:26px; color: green;">Has Been Send! <br> '.$data['notes'].' <br> Thank you!</span>
                <br /><br />
                </td>
            </tr>';
            } elseif($data['statusOrder'] == 4) {
